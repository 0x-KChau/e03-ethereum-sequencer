...
# Kill background processes when the script is killed
trap "kill 0" EXIT
...
rm -rf "$absdir/testnet"
mkdir "$absdir/testnet"
...
cd "$absdir/testnet"
for i in "${nodes[@]}"; do
  # If there are some keys in `ibft-testnet/nodeX`, reuse them
...
if [[ -d ""$absdir/tls"" ]]; then
  ...
  ./generate-tls.sh "DNS:localhost,DNS:besu-1,DNS:besu-2,IP:127.0.0.1" "DNS:localhost,DNS:enterprise-canton,IP:127.0.0.1"
fi
...
besu rlp encode --from=testnet/toEncode.json --to=testnet/encodedExtraData
...
echo "Regenerating the genesis file"
